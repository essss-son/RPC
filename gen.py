...
def generate_multi(args):
    f = open("../test_data/finetune_{}_{}.jsonl".format(args.task_mode, args.length), 'w')
                    ...
                    while cur_len < max_length:
                        for item in args.att_type:
                            ...
                            att_dic[item]['dict'] = model(input_ids=input_text, return_dict=True, use_cache=True,
                                                              use_prefix=True, prefix_id=prefix_id, insert_sign=False)
                            att_dic[item]['logits'] = att_dic[item]['dict'].logits[:, -1, :]
                            att_dic[item]['past_kv'] = att_dic[item]['dict'].past_key_values
                            att_dic[item]['logits_norm'] = torch.softmax(att_dic[item]['logits'], dim=-1)


                            satt_dic[item]['next_token_logitsatt_dic'] = logits_norm_base * (cond_att_logits ** args.lambda_cs)
