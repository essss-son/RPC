...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, is_cross_attention=False):
        ...
        try: 
            ...
            if config.prefix_mid_size:
                self.prefix_mid_size = config.prefix_mid_size
            self.prefix_keys_embeddings_n0 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_values_embeddings_n0 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_keys_embeddings_n1 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_values_embeddings_n1 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_keys_embeddings_n2 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_values_embeddings_n2 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_keys_embeddings_n3 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_values_embeddings_n3 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_keys_embeddings_n4 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_values_embeddings_n4 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_keys_embeddings_n5 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_values_embeddings_n5 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_keys_embeddings_n6 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_values_embeddings_n6 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_keys_embeddings_n7 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_values_embeddings_n7 = nn.Embedding(config.prefix_len, config.prefix_mid_size)
            self.prefix_mlp_n0 = Conv1D(config.n_embd, config.prefix_mid_size)
            self.prefix_mlp_n1 = Conv1D(config.n_embd, config.prefix_mid_size)
            self.prefix_mlp_n2 = Conv1D(config.n_embd, config.prefix_mid_size)
            self.prefix_mlp_n3 = Conv1D(config.n_embd, config.prefix_mid_size)
            self.prefix_mlp_n4 = Conv1D(config.n_embd, config.prefix_mid_size)
            self.prefix_mlp_n5 = Conv1D(config.n_embd, config.prefix_mid_size)
            self.prefix_mlp_n6 = Conv1D(config.n_embd, config.prefix_mid_size)
            self.prefix_mlp_n7 = Conv1D(config.n_embd, config.prefix_mid_size)
            self.prefix_mlp_n0 = nn.Sequential(
                nn.Linear(self.prefix_mid_size, self.prefix_mid_size * 2),
                nn.ReLU(),
                nn.Linear(self.prefix_mid_size * 2, self.prefix_mid_size * 2)
                )
            self.prefix_mlp_n1 = nn.Sequential(
                nn.Linear(self.prefix_mid_size, self.prefix_mid_size * 2),
                nn.ReLU(),
                nn.Linear(self.prefix_mid_size * 2, self.prefix_mid_size * 2)
                )
            self.prefix_mlp_n2 = nn.Sequential(
                nn.Linear(self.prefix_mid_size, self.prefix_mid_size * 2),
                nn.ReLU(),
                nn.Linear(self.prefix_mid_size * 2, self.prefix_mid_size * 2)
                )
            self.prefix_mlp_n3 = nn.Sequential(
                nn.Linear(self.prefix_mid_size, self.prefix_mid_size * 2),
                nn.ReLU(),
                nn.Linear(self.prefix_mid_size * 2, self.prefix_mid_size * 2)
                )
            self.prefix_mlp_n4 = nn.Sequential(
                nn.Linear(self.prefix_mid_size, self.prefix_mid_size * 2),
                nn.ReLU(),
                nn.Linear(self.prefix_mid_size * 2, self.prefix_mid_size * 2)
                )
            self.prefix_mlp_n5 = nn.Sequential(
                nn.Linear(self.prefix_mid_size, self.prefix_mid_size * 2),
                nn.ReLU(),
                nn.Linear(self.prefix_mid_size * 2, self.prefix_mid_size * 2)
                )
            self.prefix_mlp_n6 = nn.Sequential(
                nn.Linear(self.prefix_mid_size, self.prefix_mid_size * 2),
                nn.ReLU(),
                nn.Linear(self.prefix_mid_size * 2, self.prefix_mid_size * 2)
                )
            self.prefix_mlp_n7 = nn.Sequential(
                nn.Linear(self.prefix_mid_size, self.prefix_mid_size * 2),
                nn.ReLU(),
                nn.Linear(self.prefix_mid_size * 2, self.prefix_mid_size * 2)
                )
    ...
    def forward(
        ...
        if use_prefix:
            ...
            if insert_sign:
                print("injection")
                key = torch.cat((key, prefix_keys_embeddings), dim=-1)
                value = torch.cat((value, prefix_values_embeddings), dim=-2)
                if use_cache is True:
                    present = torch.stack((key.transpose(-2, -1), value))   transpose to have same shapes for stacking
                else:
                    present = (None,)
