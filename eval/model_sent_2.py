...
class RobertaForPreTraining(RobertaPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, sentiment=None):
        ...
        tp = torch.sum(pred_sentiment.view(-1) == sentiment.view(-1)).item()
        loss_fct = nn.CrossEntropyLoss(ignore_index=-1)
        print(list((pred_sentiment.view(-1) == sentiment.view(-1))))
        tp_temp = list((pred_sentiment.view(-1) == sentiment.view(-1)))

        tp0, tp1 = 0, 0
        fp0, fp1 = 0, 0
        fn0, fn1 = 0, 0
        for i in range(batch_size):
            predicted = pred_sentiment[i].item()
            actual = sentiment[i].item()
            if tp_temp[i] == False:
        ...
        return loss, tp, tp0, tp1, fp0, fp1, fn0, fn1
