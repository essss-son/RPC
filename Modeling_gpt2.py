...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, is_cross_attention=False):
        ...
        try:  # note prefix_len 20  prefix_mid_size 512
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False, use_prefix=False):
        w = torch.matmul(q, k)
        if self.scale:
    ...
    def forward(
        ...
        attn_outputs = self._attn(query, key, value, attention_mask, head_mask, output_attentions,use_prefix)
        a = attn_outputs[0]
